...
async function getDropdownSuggestions(prompt: string): Promise<string[]> {
    return new Promise((resolve) => {
        const scriptPath = path.resolve(__dirname, '..', 'bytebandits', 'core', 'inferenceDropDown.py');
...
\`\`\``;
    ...
    // Inline completion provider for Python
    const inlineProvider: vscode.InlineCompletionItemProvider = {
        async provideInlineCompletionItems(document, position, context, token) {
            ...
            if (linePrefix.trim() === '') {
                console.log('[Inline] Empty line, returning empty suggestions');
                return { items: [] };
            }
            ...
            try {
                ...
                console.error('[Inline] Error getting suggestions:', error);
                return { items: [] };
            }
        },
    };
